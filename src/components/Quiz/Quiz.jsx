...
function Quiz() {
  ...
  const [quizCompleted, setQuizCompleted] = useState(false);
  const [isFinalQuestion, setIsFinalQuestion] = useState(false);
  const [timeLeft, setTimeLeft] = useState(10); // Đặt thời gian mặc định (300 giây = 5 phút)
  const question = questionsData[index];
  ...
  useEffect(() => {
    if (timeLeft > 0 && !quizCompleted) {
      ...
      return () => clearTimeout(timer); // Xóa timer khi component unmount hoặc khi thời gian cập nhật
    } else if (timeLeft === 0) {
      handleComplete(); // Hoàn thành trò chơi khi hết giờ
    }
